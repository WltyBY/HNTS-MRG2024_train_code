...
class NoMirrorVNetTrainer(nnUNetTrainer):
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        """
        path = "./nnUNet/nnunetv2/training/nnUNetTrainer/DFUNet/NetConfig.yaml"
        Network_setting = open_yaml(path)
        Network_setting["SegRapVNet"]["in_channels"] = num_input_channels
        Network_setting["SegRapVNet"]["out_channels"] = num_output_channels
        return VNet(Network_setting["SegRapVNet"])
    ...
    def _get_deep_supervision_scales(self):
        ...
        if self.enable_deep_supervision:
            deep_supervision_scales = list(list(i) for i in 1 / np.cumprod(np.vstack(
                [[1, 1, 1]] + Network_setting["SegRapVNet"]["pool_kernel_size"]), axis=0))[:-1]
        else:
