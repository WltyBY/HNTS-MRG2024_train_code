...
class LG_CutMixTrainer(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.cutmix_loss = DC_and_CE_loss(
                {
    ...
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        ...
        if dim == 2:
            ...
            do_dummy_2d_aug = (max(patch_size) / patch_size[0]) > ANISO_THRESHOLD
            if do_dummy_2d_aug:
        ...
        self.print_to_log_file(f"do_dummy_2d_aug: {do_dummy_2d_aug}")
        self.inference_allowed_mirroring_axes = mirror_axes

        return rotation_for_DA, do_dummy_2d_aug, initial_patch_size, mirror_axes
    ...
    def on_train_start(self):
        ...
        # print(f"batch size: {self.batch_MixUpTrainer.py
    ...
    def _get_index_range(self, shape, patch_size, half_patch_size, dim):
        if dim == 2:
            index_range_low = half_patch_size
            index_range_up = shape - patch_size + half_patch_size
        elif dim == 3:
            # do normal sample, limit every axis range
            index_range_low = np.array([half_patch_size[i] for i in range(dim)])
            index_range_up = np.array(
                [shape[i] - patch_size[i] + half_patch_size[i] for i in range(dim)]
            )
        else:
    ...
    def cutmix(self, data, target, box_size):
        shape = data.shape[2:]
        dim = len(shape)
        half_box_size = box_size // 2
        index_range_low, index_range_up = self._get_index_range(
            shape, box_size, half_box_size, dim
        )
        sampled_voxel = [
            torch.tensor(
                [
                    torch.randint(index_range_low[i], index_range_up[i], (1,))
                    for i in range(len(index_range_low))
                ]
            )
            for _ in range(self.batch_size)
        ]
        bbmin = [sampled_voxel[i] - half_box_size for i in range(self.batch_size)]
        bbmax = [bbmin[i] + box_size for i in range(self.batch_size)]
        with torch.no_grad():
            index = torch.randperm(self.batch_size)
            while all(index == torch.tensor([i for i in range(self.batch_size)])):
                index = torch.randperm(self.batch_size)

            mask = torch.zeros((self.batch_size, 1, *shape), device=self.device)
            ...
            mixed_data = data * mask + data[index, :] * (1 - mask)
            mixed_target = target * mask + target[index, :] * (1 - mask)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with (
            ...
            if random.random() < 0.5:
                # Do Cutmix, input must be ont-hot label
                cutmix_data, cutmix_target = self.cutmix(
                    data,
                    target[0],
                    torch.tensor(self.configuration_manager.patch_size) // 2,
                )

                cutmix_output = self.network(cutmix_data)
                # del data
                l = self.cutmix_loss(cutmix_output[0], cutmix_target)
            else:
                ...
                if next_stages is not None:
                    for n in next_stages:
                        ...
                        expected_preprocessed_folder = join(
                            nnUNet_preprocessed,
                            self.plans_manager.dataset_name,
                            next_stage_config_manager.identifier,
                        )
