...
def run_a_case(
    ...
    label_path = os.path.join(label_folder, case)

    img_path = os.path.join(data_path, "image.nii.gz")
    img_save_path = os.path.join(
        img_save_folder, "MixSegRap_{:0>4d}_0000.nii.gz".format(idx)
    )

    contrast_img_path = os.path.join(data_path, "image_contrast.nii.gz")
    contrast_img_save_path = os.path.join(
        img_save_folder, "MixSegRap_{:0>4d}_0000.nii.gz".format(idx + 1)
    )

    all_Label_files = os.listdir(label_path)
    all_Label_files.sort()
    all_Label_paths = [os.path.join(label_path, i) for i in all_Label_files]
    seg_array, class2number = get_seg(all_Label_paths)

    if os.path.isfile(class2number_path):
        former_one = open_json(class2number_path)
        if class2number != former_one:
            print("Class and its corresponding number should be same for every case.", case)
            pprint(class2number)
            pprint(former_one)
    else:
        save_json(class2number, class2number_path)
    ...
    CT_seg_save_path = os.path.join(
        seg_save_folder, "MixSegRap_{:0>4d}.nii.gz".format(idx)
    )
    Enhanced_CT_seg_save_path = os.path.join(
        seg_save_folder, "MixSegRap_{:0>4d}.nii.gz".format(idx + 1)
    )
...
if __name__ == "__main__":
    ...
    label_folder = "/media/x/Wlty/LymphNodes/Dataset/SegRap2023_raw/SegRap2023_Training_Set_120cases_Update_Labels"
    save_folder = "/media/x/Wlty/nnUNet_workspace/nnUNet_raw/Dataset120_MixSegRap"

    img_save_folder = os.path.join(save_folder, "imagesTr")
    seg_save_folder = os.path.join(save_folder, "labelsTr")
    class2number_path = os.path.join(save_folder, "class2number.json")
