...
from nnunetv2.training.nnUNetTrainer.DFUNet.DFUNet import VNet, open_yaml


class MixUpVNetTrainer(nnUNetTrainer):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.mixup_loss = nn.CrossEntropyLoss()
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        Network_setting["VNet"]["in_channels"] = num_input_channels
        Network_setting["VNet"]["out_channels"] = num_output_channels
        return VNet(Network_setting["VNet"])
    ...
    def _get_deep_supervision_scales(self):
        ...
        if self.enable_deep_supervision:
            deep_supervision_scales = list(list(i) for i in 1 / np.cumprod(np.vstack(
                [[1, 1, 1]] + Network_setting["VNet"]["pool_kernel_size"]), axis=0))[:-1]
        else:
    ...
    def trans2onehot(self, data, target):
        with torch.no_grad():
            if data.ndim != target.ndim:
            ...
            if target_sample.shape == target.shape:
                # if this is the case then gt is probably already a one hot encoding
                y_onehot = target
            else:
                y_onehot = torch.zeros(
                    target_sample.shape, device=target.device, dtype=torch.float
                )
                y_onehot.scatter_(1, target.long(), 1)

        return y_onehot

    def mixup(self, data, target, alpha=0.4):
        with torch.no_grad():
            # data and target in [b, c, z, y, x]
            lamb = torch.clamp(
                Beta(alpha, alpha).rsample(
                    (self.batch_size, *[1 for _ in range(len(data.shape) - 1)])
                ),
                min=1e-4,
                max=1 - 1e-4,
            ).to(self.device)
            ...
            mixed_target = lamb * self.trans2onehot(data, target) + (
                1 - lamb
            ) * self.trans2onehot(data, target[index])
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            # Do mixup, input must be ont-hot label
            mixup_data, mixup_target = self.mixup(data, target[0])

            output = self.network(data)
            mixup_output = self.network(mixup_data)
            # del data
            l = self.loss(output["pred"], target) + self.mixup_loss(mixup_output["pred"][0], mixup_target)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output = self.network(data)
            del data
            l = self.loss(output["pred"], target)
